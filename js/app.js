(() => {
    ...
    function initSliders() {
        if (document.querySelector(".cards__slider")) new core(".cards__slider", {
            ...
            speed: 300,
            pagination: {
                el: ".controls-slider-cards__dotts",
                clickable: true
            },
            navigation: {
    ...
    da.init();
    document.querySelector(".aside__items");
    const script_link = document.querySelectorAll(".aside__link");
    for (var i = 0; i < script_link.length; i++) script_link[i].addEventListener("click", (function() {
        var current = document.getElementsByClassName("active");
        current[0].className = current[0].className.replace(" active", "");
        this.className += " active";
    }));
    const notBtn = document.querySelector(".notification-header__dots");
    const notList = document.querySelector(".notification-header__list");
    const closeDotBtn = document.querySelector(".dots-close");
    notBtn.addEventListener("click", (function() {
        notList.classList.toggle("open-list");
        closeDotBtn.classList.toggle("dots-active");
    }));
    const optionBtn = document.querySelectorAll(".options-cards__btn");
    const optionList = document.querySelectorAll(".options-cards__list");
    if (optionBtn) optionBtn.forEach((optBtn => {
        optBtn.addEventListener("click", (() => {
            optBtn.classList.toggle("card-btn-close");
            for (i = 0; i < optionList.length; i++) optionList[i].classList.toggle("options-list-close");
        }));
    }));
    const obctButton = document.querySelector(".top-objects__button");
    const obctLabesl = document.querySelectorAll(".checkbox__label");
    if (obctButton) obctButton.addEventListener("click", (() => {
        obctButton.classList.toggle("objects-button-active");
        obctLabesl.forEach((items => {
            items.classList.toggle("objects-label-active");
        }));
    }));
    const coverList = document.querySelector(".cover-main");
    const coverBtn = document.querySelector(".create-button");
    const coverBtnClose = document.querySelector(".cover-main__button");
    if (coverBtn) coverBtn.addEventListener("click", (() => {
        coverList.classList.add("cover-open");
    }));
    if (coverBtnClose) coverBtnClose.addEventListener("click", (() => {
        coverList.classList.remove("cover-open");
    }));
    const photosFinder = document.querySelector(".main-photos__folder");
    const photosCover = document.querySelector(".photos__cover");
    const photosBtn = document.querySelector(".cover-photos__btn");
    if (photosFinder) photosFinder.addEventListener("click", (() => {
        photosCover.classList.add("cover-photos-open");
    }));
    if (photosBtn) photosBtn.addEventListener("click", (() => {
        photosCover.classList.remove("cover-photos-open");
    }));
    document.querySelectorAll(".cover-photos__item");
    const photoItemBtn = document.querySelectorAll(".cover-photos__button");
    const photoItemBtnAll = document.querySelector(".cover-photos__selectbtn");
    photoItemBtn.forEach((btn => {
        btn.addEventListener("click", (() => {
            btn.classList.toggle("photos-button-active");
        }));
    }));
    if (photoItemBtnAll) photoItemBtnAll.addEventListener("click", (() => {
        photoItemBtnAll.classList.toggle("photod-selectbtn-active");
        photoItemBtn.forEach((btn => {
            btn.classList.toggle("photos-button-active");
        }));
    }));
